...
export default async function SendMail(to: string, subject: string, text: string, username: string) {
    try {
        const mailOptions = {
...
export async function SendRecoveryMail(to: string, subject: string, text: string, username: string) {
    try {
        const mailOptions = {
            from: process.env.SMTP_USER,
            to: to,
